...
class ResConfigSettings(models.TransientModel):
    ...
    app_system_name = fields.Char('System Name', help="Setup System Name,which replace Odoo",
                                  default='Pandora', config_parameter='app_system_name')
    ...
    group_show_author_in_apps = fields.Boolean(string="Show Author in Apps Dashboard", implied_group='app_pandora_customize.group_show_author_in_apps',
    ...
    app_debug_only_admin = fields.Boolean('Debug for Admin', config_parameter='app_debug_only_admin',
                                          help="Check to only Debug / Debug Assets for Pandora Admin. Deny debug from url for other user.")
    app_stop_subscribe = fields.Boolean('Stop Pandora Subscribe', help="Check to stop subscribe and follow. This to make Pandora speed up.",
                                        config_parameter='app_stop_subscribe')
    # 处理额外模块
    module_app_odoo_doc = fields.Boolean("Help Document Anywhere", help='Get Help Documentation on current Pandora operation or topic.')
    
    # 应用帮助文档
    app_doc_root_url = fields.Char('Help of topic domain', config_parameter='app_doc_root_url', default='https://pandorarevolution.com')
    ...
    def set_module_url(self, rec=None):
        ...
        app_enterprise_url = config_parameter.get_param('app_enterprise_url', 'https://www.pandorarevolution.com')
        modules = self.env['ir.module.module'].search([('license', 'like',), ('website', '!=', False)])
        if modules:
